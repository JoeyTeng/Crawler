...
def url(_url, params=None, domain=None, parser=None):
    ...
    if parser == 'rss':
        interpreter.send_task(('%s.actor.rss' % PROJECT_NAME),
                              args=args, kwargs=kwargs, routing_key='rss', queue='rss')
    elif parser == 'webpage':
        interpreter.send_task(('%s.actor.webpage' % PROJECT_NAME), args=args,
                              kwargs=kwargs, routing_key='webpage', queue='webpage')
