...
def parse(data, template=default.parser.template, config=default.parser.config, parser=None):
    # should return (args, kwargs) which fits 'interpreter.url'
    for args, kwargs in _parser.parse(data, template=template, config=config, parser=parser):
        actor.send_task(('%s.interpreter.url' % PROJECT_NAME),
                        args=args, kwargs=kwargs, routing_key='url', queue='url')
